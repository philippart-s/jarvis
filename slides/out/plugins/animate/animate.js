  ...
  if (window.slidesk.currentSlide !== 0) {
    const prevHashes = [
      ...document.querySelectorAll(
        "section.sd-previous.animate [data-animhash]"
      ),
    ].map((e) => e.getAttribute("data-animhash"));
    const currentHashes = [
      ...document.querySelectorAll(
        "section.sd-current.animate [data-animhash]"
      ),
    ].map((e) => e.getAttribute("data-animhash"));
    currentHashes
      ...
      .forEach((intersection) => {
        ...
        animation({
          ...
          draw: (progress) => {
            ghost.style.top = `${
              pBCR.top + progress * (cBCR.top - pBCR.top)
            }px`;
            ghost.style.left = `${
              pBCR.left + progress * (cBCR.left - pBCR.left)
            }px`;
            ghost.style.bottom = `${
              pBCR.bottom + progress * (cBCR.bottom - pBCR.bottom)
            }px`;
            ghost.style.right = `${
              pBCR.right + progress * (cBCR.right - pBCR.right)
            }px`;
            ghost.style.width = `${
              pBCR.width + progress * (cBCR.width - pBCR.width)
            }px`;
            ghost.style.height = `${
              pBCR.height + progress * (cBCR.height - pBCR.height)
            }px`;
          },
          done: () => {
